...
class MainWindow(QMainWindow):
    ...
    def open_settings(self):
        ...
        # シグナル接続をダイアログ表示前に確実に行う
        dialog.settings_updated.connect(self.refresh_metadata)
        print("Connected settings_updated signal to refresh_metadata")  # デバッグ出力
        dialog.templates_updated.connect(self.word_blocks.refresh_templates)
        print("Connected templates_updated signal to word_blocks.refresh_templates")  # デバッグ出力
        # シグナルが発行されたことを確認するためのデバッグ接続
        dialog.templates_updated.connect(lambda: print("templates_updated signal received in MainWindow"))
        dialog.exec_()
        print("Settings dialog closed")  # デバッグ出力