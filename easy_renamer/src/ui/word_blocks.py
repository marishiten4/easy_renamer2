...
class WordBlocks(QWidget):
    ...
    def update_candidates(self, metadata):
        ...
        for key, value in metadata.items():
            # 'no_match' キーを除外
            if key == "no_match":
                print(f"Skipping 'no_match' key with value: {value}")  # デバッグ出力
                continue
            # 空、None、"None"文字列、空白のみ、"一致なし"を除外
    ...
    def update_pattern(self, template):
        print(f"Updating pattern with template: {template}")  # デバッグ出力
        if template != "カスタム":
    ...
    def refresh_templates(self):
        ...
        if templates:
            ...
            print(f"Selected template after refresh: {selected_template}")  # デバッグ出力
            self.update_pattern(selected_template)
        else:
            print("No templates available to select.")  # デバッグ出力