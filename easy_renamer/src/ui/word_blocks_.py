...
class WordBlocks(QWidget):
    ...
    def update_candidates(self, metadata):
        ...
        print(f"Predefined words: {predefined_words}")
        ...
        for key, value in metadata.items():
            if key == "no_match":
                print(f"Skipping 'no_match' key with value: {value}")
                continue
        ...
        print(f"Filtered metadata words: {metadata_words}")
    ...
    def update_pattern(self, template):
        print(f"Updating pattern with template: {template}")
        if template != "カスタム":
    ...
    def add_sequence(self):
        ...
        start_number = self.number_input.text() or "001"
        sequence_text = f"{fixed_part}{{連番}}"
    ...
    def refresh_templates(self):
        ...
        print(f"Looking for settings.json at: {settings_path}")
        try:
        ...
        print(f"Loaded templates for combo: {templates}")
        if not templates:
            print("Warning: No templates loaded. Check settings.json.")
            self.template_combo.addItem("No templates available")
        else:
            self.template_combo.addItems(templates)
            print(f"Added {len(templates)} templates to combo box.")

        self.template_combo.setCurrentIndex(-1)
        print("Templates refreshed, no template selected.")